...
def calc_raw_attribution(output_dir, session_id):
    ...
    for chani in range(num_channels):
        ...
        attrs = ig.run(X_attr, baselines=0, n_batch=15, n_steps=10).cpu()
        ...
        filename = Path(output_dir_attrs / f'attribution_scores_raw_entire_session_chan{chani}_band{bandi}.npz')
    ...
    print("Session raw attribution scores are saved in: ", output_dir_attrs)
    print("Session raw attribution analysis complete.", flush=True)
...
def main(args):
    ...
    calc_raw_attribution(root_dir, session_id)
