...
def start_spontaneous_attribution(output_dir, session_id):
    ...
    lfp_obj = pre_process_lfp(session_id, session_obj.channels, session_obj.spontaneous_times[0],
    ...
    for bandi in range(len(bands)+1):
        for chani in range(num_channels):
            ...
            filename = Path(output_dir_attrs / f'attribution_scores_spontaneous_chan{chani}_band{bandi}.npz')
    ...
    filename = Path(output_dir_attrs / f'attribution_scores_spontaneous_mean.npy')
    np.save(filename, mean_attribution)
    print("Spontaneous attribution scores are saved in: ", output_dir_attrs)
    print("Spontaneous attribution analysis complete.", flush=True)
...
def main(args):
    ...
    start_spontaneous_attribution(root_dir, session_id)
