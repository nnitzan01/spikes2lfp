...
def calculate_ipd_hvs(output_dir, session_id):
    ...
    rank_snippets = np.load(variables_dir / 'rank_snippets_hvs.npy')
    
    # Load and reconstruct Sparse Active Mask
    sparse_mask = load_npz(variables_dir / 'active_mask_snippet_sparse_hvs.npz')
    original_shape = np.load(variables_dir / 'active_mask_shape_hvs.npy')
    ...
    print(f"Calculating IPD for {n_regions} regions across {n_trials} HVS events and {n_channels} channels.", flush=True)
    ...
    # --- 4. Save Results ---
    output_path = variables_dir / 'regional_dominance_results_hvs.npz'
    np.savez_compressed(
        output_path,
...
def main(args):
    ...
    calculate_ipd_hvs(root_dir, session_id)
