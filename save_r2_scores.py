...
def start(output_dir, session_id):
    ...
    spikes_obj_active = pre_process_spikes(session_obj.units, session_obj.spike_times, bin_size=bin_size, sigma=3)
    spikes_obj_active.getSpkMat(session_obj.active_times[0], session_obj.active_times[1])
    spikes_obj_active.truncate(seqlength)
    ...
    R2_test_active  = np.zeros((len(lfp_obj_active.channels), len(bands)+1))
    ...
    for bandi in range(len(bands)+1):
        X_train_active, X_test_active, y_train_active, y_test_active = chunk_and_reshape(spikes_obj_active.spkMat, lfp_obj_active.lfpMat[:,:,bandi],
        ...
        for chani in range(len(lfp_obj_active.channels)):
            _, test_dataloader_active    = get_data_loaders(X_train_active, X_test_active, y_train_active[:,:,chani], y_test_active[:,:,chani], batch_size)
            ...
            yHat = model.evaluate(test_dataloader_active.dataset.tensors[0])
            R2_test_active[chani, bandi] = r2_score(test_dataloader_active.dataset.tensors[1].cpu().numpy().reshape(-1,1), np.array(yHat.to('cpu')))
    ...
    r2_scores_active_path = Path(variables_dir / 'r2_scores.npy')
    np.save(r2_scores_active_path, R2_test_active)
