...
def save_spont_instantaneous_predictive_influence(output_dir, session_id):
    ...
    mean_attribution_raw = np.load(output_dir_attrs / f'attribution_scores_spontaneous_mean.npy')
    ...
    for bandi in range(len(bands)+1):
        for chani in range(num_channels):
            ...
            sparse_attr = np.load(output_dir_attrs / f'attribution_scores_spontaneous_chan{chani}_band{bandi}.npz')
    ...
    filename = Path(output_dir_attrs / f'attribution_scores_spontaneous_ipi_mean.npy')
    np.save(filename, mean_attribution_ipi)
    print("Relative IPI spontaneous attribution scores are saved in: ", output_dir_attrs)
...
def main(args):
    ...
    save_spont_instantaneous_predictive_influence(root_dir, session_id)
